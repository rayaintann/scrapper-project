-- =====================================================================
-- 008_sync_procedures_with_avatar_website.sql
--
-- Lanjutan migration 007. Setelah kolom baru ada:
--   l0_harmonization.instagram_profile.avatar_url
--   l0_harmonization.tiktok_profile.website
-- procedure sinkronisasi harus ikut mengisinya, supaya baris BARU dari
-- ingest berikutnya tidak kembali NULL. Backfill di 007 hanya menutup
-- baris yang sudah ada saat ini.
--
-- Isi kedua procedure sama persis dengan 004/005, HANYA menambah satu
-- kolom di masing-masing:
--   instagram: avatar_url <- COALESCE(raw_payload->>'profilePicUrlHD',
--                                     raw_payload->>'profilePicUrl')
--   tiktok   : website    <- authorMeta.bioLink, diberi prefix https://
--                            bila belum berskema
--
-- Blok 'official' memakai NULL untuk kedua kolom itu karena tabel
-- l0_raw.ig_profile_official / tt_profile_official memang tidak
-- menyediakan sumbernya. Klausa COALESCE di DO UPDATE menjaga nilai
-- dari apify supaya tidak tertimpa NULL.
--
...
BEGIN
  ...
  INSERT INTO l0_harmonization.tiktok_profile (
    ...
    avatar_url, website, is_verified, follower_count, following_count, likes_count,
